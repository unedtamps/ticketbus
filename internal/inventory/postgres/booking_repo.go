...
func (r *BookingRepo) Create(ctx context.Context, b *domain.Booking) error {
	...
	_, err = tx.Exec(ctx, `
		INSERT INTO bookings (id, user_id, event_id, status, total_cents, payment_id)
		VALUES ($1,$2,$3,$4,$5,$6)`,
		b.ID, b.UserID, b.EventID, b.Status, b.TotalCents, b.PaymentID)
	if err != nil {
		return err
	}
...
func (r *BookingRepo) FindByID(ctx context.Context, id string) (*domain.Booking, error) {
	...
	err := r.pool.QueryRow(ctx, `SELECT id, user_id, event_id, status, total_cents, payment_id, created_at FROM bookings WHERE id=$1`, id).
		Scan(&b.ID, &b.UserID, &b.EventID, &b.Status, &b.TotalCents, &b.PaymentID, &b.CreatedAt)
	if err != nil {
		return nil, err
	}
...
func (r *BookingRepo) ListByUser(ctx context.Context, userID string) ([]domain.Booking, error) {
	rows, err := r.pool.Query(ctx, `SELECT id, user_id, event_id, status, total_cents, payment_id, created_at FROM bookings WHERE user_id=$1 ORDER BY created_at DESC`, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var b domain.Booking
		if err := rows.Scan(&b.ID, &b.UserID, &b.EventID, &b.Status, &b.TotalCents, &b.PaymentID, &b.CreatedAt); err != nil {
			return nil, err
		}
